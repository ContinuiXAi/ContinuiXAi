...
## Database validation gate — currently blocked

The forward migration `20260917130000_inventory_history_index` adds the provisional, **unmeasured** index `(organizationId, siteId, productId, occurredAt, createdAt)`. Correctness does not depend on it. No historical migration or ledger event is modified. Normal `CREATE INDEX` can block writes: deployment scheduling requires separate approval.
...
It then adds 300,000 synthetic events across 400 products, three sites and two tenants, analyzes the table, and prints 36 `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` plans for one/50/100-product pages, selective/broad effective cutoffs, and optional recorded cutoffs. Candidate, existing-index baseline, and effective-time-first alternatives are compared. Alternative DDL is transactional and rolled back; ledger protection is never disabled. Retain the output and review timing/buffer usage before confirming index order. Do not interpret a generated script as executed evidence.

Fixtures retain immutable history. Destroy the entire disposable database after retaining evidence; the script never deletes protected ledger records. No PostgreSQL execution, EXPLAIN evidence, or live migration validation was available in the authoring environment.
