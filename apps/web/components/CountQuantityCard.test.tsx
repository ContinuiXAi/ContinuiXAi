...
describe("CountQuantityCard", () => {
  ...
  it("shows product identity, location, quantity controls, and clear actions", async () => {
    ...
    expect(input().labels?.[0].textContent).toBe("Units to add");
    expect(container.textContent).toContain("Enter only the units you are adding now.");
    expect(container.textContent).not.toContain("Enter the full quantity");
